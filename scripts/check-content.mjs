...
for(const p of papers){if(known.has(p.id))errors.push(`Case-insensitive ID collision: ${p.id}`);known.set(p.id,p);if(!p.data.published)continue;
 ...
 for(const f of p.data.figures??[])if(f.id&&(!f.imageVerified||!f.sha256))errors.push(`${p.id}: unverified figure ${f.id}`);
}
let published=0;
for(const d of issues){if(!d.data.published)continue;published++;const ids=d.data.papers??[];
 ...
 for(const id of ids){if(id!==id.toLowerCase())errors.push(`${d.id}: non-canonical ID ${id}`);if(!known.get(id)?.data.published)errors.push(`${d.id}: missing/unpublished ${id}`);else if(new Date(known.get(id).data.date)>new Date(d.data.date))errors.push(`${d.id}: future paper ${id}`);}
}
