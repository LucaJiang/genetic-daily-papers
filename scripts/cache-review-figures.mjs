...
async function imageURL(f){if(f.url)return f.url;const {r}=await get(f.sourceUrl),text=await r.text();const figNo=f.sourceUrl.match(/\/figures\/(\d+)/)?.[1];
 ...
 if(!selected)throw new Error(`No Figure ${figNo} image URL found; inspect the source PDF for ${f.assetPath}`);
