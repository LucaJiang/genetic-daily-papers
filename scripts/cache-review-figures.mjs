/** Fetch licensed, unchanged figures; never replace a failed download with a placeholder. */
...
async function imageURL(f){if(f.url)return f.url.replace(/\/(?:lw|m)\d+\//,'/full/');const {r}=await get(f.sourceUrl),text=await r.text();const figNo=f.sourceUrl.match(/\/figures\/(\d+)/)?.[1];
 ...
 if(!selected)throw new Error(`No verified Figure ${figNo} image URL on publisher page; inspect the article PDF and supply the unchanged figure at ${f.assetPath}`);
 return selected.replace(/\/m\d+\//,'/full/').replace(/\/lw\d+\//,'/full/');
}
const referenced=new Set();
for(const file of await readdir('src/content/daily')){
 if(!file.endsWith('.md'))continue;
 const raw=await readFile(join('src/content/daily',file),'utf8'),fm=raw.match(/^---\r?\n([\s\S]*?)\r?\n---/);
 if(!fm)continue;
 const meta=parse(fm[1]);
 if(meta.published)for(const id of meta.papers??[])referenced.add(String(id).toLowerCase());
}
const all=new Map();
for(const file of await readdir('src/content/papers')){
 if(!file.endsWith('.md'))continue;
 const id=basename(file,'.md').toLowerCase();
 if(!referenced.has(id))continue;
 const raw=await readFile(join('src/content/papers',file),'utf8'),fm=raw.match(/^---\r?\n([\s\S]*?)\r?\n---/);
 if(!fm)continue;
 const meta=parse(fm[1]);
 if(!meta.published)throw new Error(`Referenced paper ${id} is not published`);
 for(const f of figuresFor(meta,id))all.set(assetFor(f),f);
}
await mkdir('public/figures',{recursive:true});await mkdir('.radar',{recursive:true});const report=[];const errors=[];
for(const [asset,f] of all){const dest=join('public',asset.slice(1));try{let bytes,url=f.url??f.sourceUrl,cached=false;try{if((await stat(dest)).size>1000){bytes=await readFile(dest);cached=true;}}catch{}
 ...
 report.push({asset,sourceUrl:f.sourceUrl,retrievedUrl:url,sha256:createHash('sha256').update(bytes).digest('hex'),bytes:bytes.length,...info,cached});
 console.log(`[figure] ${cached?'local':'fetched'} ${asset}`);
...
if(errors.length){console.error(`${errors.length} figures missing. No placeholder or remote fallback was published. See docs/ASSET_CHECKS.md.`);process.exit(1);}console.log(`[figures] ${report.length}/${all.size} image files verified`);
