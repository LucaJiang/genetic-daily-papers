...
type SourceFigure=Omit<PaperFigure,'url'>&{url?:string};
export function figuresForPaper(paper:CollectionEntry<'papers'>):PaperFigure[]{const figures=figuresFor(paper.data,paper.id) as SourceFigure[];return figures.map(f=>({...f,url:f.url??f.sourceUrl}));}
