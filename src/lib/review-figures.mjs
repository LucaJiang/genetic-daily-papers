...
export function figuresFor(meta,slug){
 ...
 return list.map((f,i)=>({...f,id:f.id??('fig'+(f.sourceUrl?.match(/\/figures\/(\d+)/)?.[1]??i+1)),
   label:(f.label??`Figure ${i+1}`).replace(/^(真实数据|方法流程|模拟|数据库|验证|方法)[：:]\s*/u,''),
   kind:f.kind??'validation',assetPath:assetFor(f)})).sort((a,b)=>(rank[a.kind]??5)-(rank[b.kind]??5));
}
