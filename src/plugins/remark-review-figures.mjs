...
export default function remarkReviewFigures(){return (tree,file)=>{
 ...
 tree.children=tree.children.map(n=>{const m=n.type==='paragraph'&&plain(n).trim().match(/^\[\[figure:([\w-]+)\]\]$/);if(!m)return n;const f=byId.get(m[1]);if(!f)throw new Error(`Unknown figure ${m[1]} in ${slug}`);explicit++;return {type:'html',value:figureHTML(f)};});
 if(explicit)return;
 ...
 let at=heading>=0?heading+1:tree.children.findIndex(n=>n.type==='heading'&&/参考|局限|小结/.test(plain(n)));if(at<0)at=tree.children.length;if(heading>=0&&tree.children[at]?.type==='paragraph')at++;
